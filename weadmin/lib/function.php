...
{
    if ($valType == 1) {
        $valRandomName = date('Ydm') . "-" . time() . "-" . rand(1111, 9999);
    } else {
        $valRandomName = date('Ydm') . "-" . time() . "-" . rand(1111, 9999);
    }
...
function loadSendEmailTo($mailTo, $mailFrom = null, $subjectMail = null, $messageMail = null)
{
    /* ############################################### */
...
###########################################
function loadGetURLByMod($valWebsite, $valLang, $valFolder, $valID, $type)
{ // หา ID Hotel ของ Webservices
    ############################################
...
############################################
function getNameGroupTable($GID, $tableDb)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
    ...
    $txt_member_name = changeQuot($row[1]);
    $textReturn = $txt_member_name;

    return $textReturn;
}
function getGidPro($Name, $tableDb)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
		" . $tableDb . "_id ,
		" . $tableDb . "_gidpro
		FROM " . $tableDb . " WHERE   " . $tableDb . "_id 	= '" . $Name . "'";
    // $query_pic=mysqli_query($sql_pic);
    // $row_pic=mysqli_fetch_array($query_pic);
...
############################################
function getNameGroupTableProvince($GID, $tableDb)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
...
############################################
function getNameGroupTableIDArray($arrID, $tableDb)
{
    ############################################
    $sql = "SELECT
    " . $tableDb . "_id ,
    " . $tableDb . "_subject
    FROM " . $tableDb . " WHERE   " . $tableDb . "_id IN (" . implode(',', $arrID) . ")";
    // $query_pic=mysqli_query($sql_pic);
    // $row_pic=mysqli_fetch_array($query_pic);
...
############################################
function getNameGroupTablePDSDArray($arrID, $tableDb)
{
    global $dbConnect;
    ############################################
    $sql = "SELECT
...
function GetGroupProvince($data = null){
    $IdPro = array(1,2,3,4,5,6);
    $namePro = array('ระยอง','สมุทรสาคร','สุราษฎร์ธานี','สงขลา','พังงา','ตรัง');
    $posPro = array('อ่าวไทยฝั่งตะวันออก','อ่าวไทยตอนบน','อ่าวไทยตอนกลาง','อ่าวไทยตอนล่าง','อันดามันตอนบน','อันดามันตอนล่าง');
    $dataPro = array();
    foreach ($namePro as $keyName => $valueName) {
        # code...
        $dataPro['id'][] = $IdPro[$keyName];
        $dataPro['pos'][] = $posPro[$keyName];
        $dataPro['name'][] = $valueName;
    }
    return $dataPro;
}

function CallGroupName($id = 0) {

    global $mod_tb_root_sub_group, $masterkey_template; 
    
    $sql_group = "SELECT ";
    $sql_group .= "  ".$mod_tb_root_sub_group."_id,".$mod_tb_root_sub_group."_subject";
    $sql_group .= "  FROM ".$mod_tb_root_sub_group." WHERE  ".$mod_tb_root_sub_group."_masterkey ='".$_REQUEST['masterkey']."' AND ".$mod_tb_root_sub_group."_id = '".$id."' ORDER BY ".$mod_tb_root_sub_group."_order DESC ";
    ...
    // print_pre($sql_group);
    return $row_groupname;
  }

function GroupName($id = 0,$masterKey,$table,$field) {

global $mod_tb_root_sub_group, $masterkey_template; 

$sql_group = "SELECT ";
$sql_group .= "  ".$table."_id,".$table."_subject, ".$table."_proid";
if(!empty($field)){
    $sql_group .= " , ".$table."_waveid";
    $fieldName = "_".$field;
}else{
    $fieldName = '_proid';
}
$sql_group .= "  FROM ".$table." WHERE  ".$table."_masterkey ='".$masterKey."' AND ".$table."".$fieldName." = '".$id."' ORDER BY ".$table."_order DESC ";
// print_pre($sql_group);die;
$query_group=wewebQueryDB($coreLanguageSQL, $sql_group);
$row_group=wewebFetchArrayDB($coreLanguageSQL,$query_group);
$row_groupid=$row_group[0];
if(!empty($field)){
    $row_groupname=$row_group[3];
}else{
    $row_groupname=$row_group[1];
}
// print_pre($sql_group);
...
function GroupNamePro($id = 0,$masterKey,$table,$field) {
...
function FindNameInArr($data = null,$index = null){
...
function GetGroupPara($data = null){
    if($data == 'cond'){
        $cond = array(
...
function GetGroupParaVal($data = null){
    if($data == 'cond'){
        $cond = array(
            '1' => 'ท้องฟ้าแจ่มใส (Clear)',
            '2' => 'มีเมฆบางส่วน (Partly cloudy)',
    ...
    if($data == 'wind'){
        $WindName = array(
            'ทิศตะวันออกเฉียงเหนือ' => '0.01-90.00',
            'ทิศตะวันออกเฉียงใต้' => '90.01-180.00',
            'ทิศตะวันตกเฉียงใต้' => '180.01-270.00',
            'ทิศตะวันตกเฉียงเหนือ' => '270.01-359.99'
        );
        $result = $WindName;
    }
    if($data == 'wave'){
...
function dailyTheme($date = null){
    global $coreLanguageSQL,$mod_tb_root,$mod_tb_root_group;
    $sql = "SELECT 
    ".$mod_tb_root.".".$mod_tb_root."_id as id,
    ".$mod_tb_root.".".$mod_tb_root."_dateitem as dateitem,
    ".$mod_tb_root.".".$mod_tb_root."_masterkey as masterkey,
    ".$mod_tb_root.".".$mod_tb_root."_status as status,
    ".$mod_tb_root.".".$mod_tb_root."_lastdate as lastdate,
    ".$mod_tb_root.".".$mod_tb_root."_groupProoject as groupProoject
    FROM 
    ".$mod_tb_root."
    WHERE
    ".$mod_tb_root.".".$mod_tb_root."_masterkey = 'daily' 
    and TO_DAYS(".$mod_tb_root.".".$mod_tb_root."_dateitem) = TO_DAYS('".$date."')
    ";
    // print_pre($sql);
    $result = wewebQueryDB($coreLanguageSQL,$sql);
    // $result = wewebFetchArrayDB($coreLanguageSQL,$query);
    return $result;
}

function shMasterkey($statusid = null){
    global $coreLanguageSQL,$mod_tb_root,$mod_tb_root_group;
    $sqlSch = "SELECT ".$mod_tb_root."_masterkey  FROM ".$mod_tb_root." WHERE  ".$mod_tb_root."_id='".$statusid."' ";
...
function DateFormatSQL($date = null){
    $dateEx = explode(' ',$date);
    return $dateEx[0];
}
...
function getFileNameReal($url,$valtype = null){
	global  $_SESSION,$valSiteManage,$core_full_path;
	
	$core_def_path = $core_full_path."/weadmin/";
	$core_def_path =str_replace($core_def_path,"",$url);
	$arrayPath = explode("/", $core_def_path);
	
	if($valtype==1){
		$datareturn = $arrayPath[0];
	}else{
	
		$arrayPhp = explode(".php", $url);
		$chk_type_php = $arrayPath[1];
		$chk_type_php = explode(".php", $chk_type_php);
		$count_type_php = count($chk_type_php);
		if($count_type_php>=2){
			$arrayPhp = explode(".php", $url);
			$arrayPath = explode("/", $arrayPhp[0]);
			$datareturn = $arrayPath[count($arrayPath) - 1]; 
		}else{
			$datareturn = $arrayPath[1];
		}
...
function file_exists_2($filePath,$fileName = null){
	if($fileName==""){
			$datareturn = false;
	}else{
