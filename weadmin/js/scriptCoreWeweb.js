...
//         zoom: 7,
//         center: { lat: 11.040548, lng: 101.829895 },
//         mapTypeId: "terrain",
//     });

//     var myMarker = new google.maps.Marker({
//         position: new google.maps.LatLng(11.040548, 101.829895),
//         map,
//         draggable: true
//     });
...
function viewContactNewMap(fileAc) {
...
function initializeView() {
    let lat_attribute = $('#use_in_js_Latitude').val();
    let long_attribute = $('#use_in_js_Longitude').val();
    let lat_marker = '';
    let long_marker = '';
    if(lat_attribute === '' && long_attribute === ''){
        lat_marker = 11.040548;
...
function delPicUploadPicEvent(fileAc) {
    ...
    jQuery.ajax({
        ...
        success: function(html) {
            ...
            jQuery("#boxPicNewPicEvent").show();
            jQuery("#boxPicNewPicEvent").html(html);
            setTimeout(jQuery.unblockUI, 200);
...
function loadData(namepro){
    let url_api_first = './api/first_API.php';
    let QueryStr = 'GetLatLong';
    console.log(namepro);
    $.ajax({
        url: url_api_first,
        data: {prov:namepro},
        success: function(res) {
            ...
            // console.log(datares);
            if(datares['WeatherForecasts'].length === 0){
                console.log('if');
                $('#latInput').val('');
                $('#longInput').val('');
            }
            if(datares['WeatherForecasts'].length !== 0){
                $('#latInput').val(datares['WeatherForecasts'][0]['location']['lat']);
                $('#longInput').val(datares['WeatherForecasts'][0]['location']['lon']);
            }
            // setTimeout(jQuery.unblockUI, 200);
            initialize();
        }
...
function oncheck(type){
  ...
  let inputType = $('input[name="inputType"]:checked').val();
  if(inputType == 'url'){
    jQuery('#boxInputlink').show();
    jQuery('#boxInputfile').hide();
  }else{
    jQuery('#boxInputfile').show();
    jQuery('#boxInputlink').hide();
  }

  if (type == 2) {
    jQuery('#boxInputType').show();
  }else{
    jQuery('#boxInputType').hide();
    jQuery('#boxInputfile').hide();
    jQuery('#boxInputlink').hide();
  }
